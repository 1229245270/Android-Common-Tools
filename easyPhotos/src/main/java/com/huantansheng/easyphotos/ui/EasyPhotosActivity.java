...
public class EasyPhotosActivity extends AppCompatActivity implements AlbumItemsAdapter.OnClickListener, PhotosAdapter.OnClickListener, AdListener, View.OnClickListener {
    ...
    /**
     * 新增方法startIntent，startActivityForResult做回调使用
     */

    public static Intent startIntent(Activity activity) {
        if (doubleClick()) return null;
        return new Intent(activity, EasyPhotosActivity.class);
    }

    public static Intent startIntent(Fragment fragment) {
        if (doubleClick()) return null;
        return new Intent(fragment.getActivity(), EasyPhotosActivity.class);
    }

    public static Intent startIntent(androidx.fragment.app.Fragment fragment) {
        if (doubleClick()) return null;
        return new Intent(fragment.getContext(), EasyPhotosActivity.class);
    }
