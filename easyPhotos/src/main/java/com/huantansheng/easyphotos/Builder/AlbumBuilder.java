...
public class AlbumBuilder {
    ...
    public Intent start(){
        setSettingParams();
        return launchEasyPhotosActivity();
    }

    private Intent launchEasyPhotosActivity() {
        if (null != mActivity && null != mActivity.get()) {
            return EasyPhotosActivity.startIntent(mActivity.get());
        }
        if (null != mFragment && null != mFragment.get()) {
            return EasyPhotosActivity.startIntent(mFragment.get());
        }
        if (null != mFragmentV && null != mFragmentV.get()) {
            return EasyPhotosActivity.startIntent(mFragmentV.get());
        }
        return null;
    }
